...
| Windows 10/11 | Qt Widgets (`CVBuilder-Qt.exe`) | собирается там, где есть Qt 6; на машине разработчика не проверялась |
...
На странице «Дополнительные задачи» включена галочка «Добавить папку программы в
PATH»: с ней `cvcli` вызывается из любой консоли просто по имени. В PATH попадает
каталог установки, а не файл; при установке для себя правится `Path` в
`HKCU\Environment`, при установке для всех — системная переменная. Запись
добавляется один раз, снятая галочка при повторной установке её убирает, и
деинсталлятор убирает тоже — остальное содержимое `PATH` не трогается. Уже
открытые окна консоли переменную не перечитывают, нужно открыть новое.
