# CV Builder — анализ переноса на Windows / Linux / macOS

Разделы 1–13 написаны **до** любых изменений кода; ссылки на строки
соответствуют состоянию дерева на коммите `a7dccb7` («Preparing for new
release»). Раздел 14 добавлен после и рассказывает, что из этого получилось,
включая те места, где реальность оказалась не такой, как ожидалось.
...
## 13. Что решено не делать (было решено до работы)
...
Всё перечисленное соблюдено: формат JSON и формат PDF не изменились, ни одна из
четырёх собственных подсистем не заменена, `layout.cpp` не тронут вообще,
Win32-фронтенд на месте.

---

## 14. Что получилось

Раздел дописан после работы. Здесь только измеренное и проверенное; там, где
проверить не удалось, так и сказано.

### 14.1. Коммиты

| Коммит | Шаг плана |
| --- | --- |
| `1c33adf` | аудит (разделы 1–13 этого файла) |
| `b31eb4a` | CMake, один на все платформы |
| `fc915bc` | локаль убрана из ядра, появился тестовый раннер |
| `431f60e` | исходники разложены по слоям, Makefile выведен из строя |
| `16471c7` | `std::filesystem::path` и файловый ввод-вывод в ядре, UI-строки из модели |
| `91cb8e2` | `resume_platform` и `resume_app` вместо реестра и shell в UI |
| `60110d5` | шрифты: core/platform/app, приложенный TTF, `.ttc` |
| `50b17dd` | `resume_render`: один обход страницы |
| `809bc96` | тесты ядра, закреплённый PDF |
| `0ba185b` | Qt-фронтенд и тест рендера предпросмотра |

### 14.2. Итоговая архитектура

```
                        CVBuilder (Win32)        CVBuilder-Qt        cvcli
                               │                      │                │
                    resume_ui_win32            resume_ui_qt            │
                               └────────┬─────────────┘                │
                                        │  resume_uicommon             │
                                        │  (палитра, подписи)          │
                                        └──────────┬───────────────────┘
                                                   │
                                              resume_app
                                   (шрифт, недавние файлы, автосейв)
                                                   │
                                    ┌──────────────┴──────────────┐
                                    │                             │
                            resume_platform                resume_render
                     (paths / settings / fonts /        (Canvas + обход
                      console; по одному файлу           страницы)
                      на ОС, выбор в CMake)                  │
                                    └──────────────┬──────────┘
                                                   │
                                             resume_core
                        (json, numeric, file, model, layout, font, pdf)
```

Проверяемое правило: `grep -r "windows.h\|_WIN32\|d2d1\|dwrite" src/core src/render`
не находит ничего.
...
| Что | Значение |
| --- | --- |
| PDF `sample_cv.json`, Windows и Linux | `6ac97f95…` на обеих, побайтово одинаков |
| Раскладка (`--dump-ops`) до и после переноса | одинакова байт в байт |
| Ширины символов Liberation Sans против Arial | совпадают во всех 918 точках |
| DejaVu Sans против Arial (для контраста) | 896 из 918 расходятся, +6 строк переноса |
| Тестов | 33 в ядре + 4 проверки рендера предпросмотра |
| `#ifdef` по платформам в `src/core`, `src/render` | 0 |
| Платформенных `#ifdef` вне `platform/` | 1 (`QT_VERSION` в `ui_qt/theme.cpp`) |

### 14.4. Где ожидания не совпали с реальностью

1. **Главной проблемой оказалась не Win32, а локаль.** Аудит нашёл это до
   переноса (§2.3), и правка стоила одного файла на сотню строк — но если бы это
   всплыло после появления Qt, симптомом был бы «PDF иногда не открывается», и
   искать причину пришлось бы долго.
2. **`FontSet::family()` был не рудиментом, а признаком дефекта.** Печать шла
   через GDI по имени семейства, то есть глифы выбирал GDI, а не наш парсер
   (§1.4). С приложенным шрифтом это стало настоящей ошибкой — такого семейства
   на Windows нет, и GDI подставил бы что попало. Печать переведена на glyph id
   (`ETO_GLYPH_INDEX` плюс шрифт, зарегистрированный в процессе приватно), и
   расхождение исчезло, а не переехало.
3. **Единая палитра оказалась нужнее, чем казалось.** Пункт 7 ТЗ («единый UX»)
   выглядел как пожелание, но два фронтенда с двумя копиями восьми цветов
   разъехались бы на первой же правке. Цвета переехали в `resume_uicommon`, а
   Qt-стиль принудительно Fusion: нативные стили перекрашивают контролы по своему
   усмотрению и половину палитры игнорируют.
4. **Абстракция рендеринга сэкономила код, а не добавила.** `Document` уже был
   нейтральным, поэтому `Canvas` — это три метода, а каждый бэкенд после
   перевода стал короче: обход страницы существует в одном месте вместо трёх.

### 14.5. Чего не хватает

* **macOS не собиралась.** Платформенный слой для неё написан и проходит
  синтаксическую проверку (`tools/macos-syntax-check.sh`), но Mac для сборки не
  было. Первое, что стоит сделать на Mac: `cmake --preset macos`,
  `ctest --preset macos` — контрольная сумма PDF в `tests/test_pdf.cpp` должна
  совпасть; если нет, почти наверняка выбран не приложенный шрифт.
* **Qt-версия под Windows не проверялась.** Кросс-компиляция из WSL невозможна:
  официальные сборки Qt под mingw используют UCRT, а mingw в репозиториях Ubuntu
  — msvcrt. Собирать нативно (MSYS2 или Qt + MSVC). Win32-версия под Windows
  работает и проверена.
* Предпросмотр остался постраничным, как в оригинале.
* Пакетов для Linux и macOS нет; установщик только под Windows.
* Тест печати отсутствует: печать проверяется только глазами, потому что
  автоматически проверить её без принтера можно лишь через PDF-принтер, что уже
  проверяет генератор PDF.
